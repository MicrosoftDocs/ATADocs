...
* **Remove Resource Based Constrained Delegation for Entra seamless SSO account**
   *  If resource-based constrained delegation is configured on the AZUREADSSOACC computer account, an account with the delegation would be able to generate service tickets for the AZUREADSSOACC account on behalf of any user and impersonate any user in the Entra tenant that is synchronized from AD. For more information click [here](remove-rbcd-microsoft-entra-seamless-single-sign-on-account.md)       
