...
Defender for Identity protects your on-premises Active Directory users and/or users synced to Microsoft Entra ID. To protect an environment made up of only Microsoft Entra ID users, see Microsoft Entra ID Protection](/azure/active-directory/identity-protection/overview).
...
- To create your Defender for Identity instance, you'll need an Microsoft Entra ID tenant with at least one global/security administrator. Each Defender for Identity instance supports a multiple Active Directory forest boundary and Forest Functional Level (FFL) of Windows 2003 and above.
