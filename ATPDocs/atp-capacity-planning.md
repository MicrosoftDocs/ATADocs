...
title: Planning your Azure Threat Protection deployment | Microsoft Docs
description: Helps you plan your deployment and decide how many ATP servers will be needed to support your network
keywords:
author: rkarlin
ms.author: rkarlin
manager: mbaldwin
...
# ATP Capacity Planning
This article helps you determine how many ATP servers are needed to monitor your network. It helps you figure out how many ATP Gateways and/or ATP Lightweight Gateways you need and the server capacity for your ATP Center and ATP Gateways.

> [!NOTE] 
> The ATP Center can be deployed on any IaaS vendor as long as the performance requirements described in this article are met.
...
- ATP Center CPU and Memory: Match the **Busy Packets/sec** field in the ATP Center table results file to the **PACKETS PER SECOND** field in the [ATP Center table](#ata-center-sizing).

- ATP Center Storage: Match the **Avg Packets/sec** field in the ATP Center table results file to the **PACKETS PER SECOND** field in the [ATP Center table](#ata-center-sizing).
- ATP Gateway: Match the **Busy Packets/sec** field in the ATP Gateway table in the results file to the **PACKETS PER SECOND** field in the [ATP Gateway table](#ata-gateway-sizing) or the [ATP Lightweight Gateway table](#ata-lightweight-gateway-sizing), depending on the [gateway type you choose](#choosing-the-right-gateway-type-for-your-deployment).
...
### ATP Center Sizing
The ATP Center requires a recommended minimum of 30 days of data for user behavioral analytics.
...
> [!NOTE]
> -   The ATP Center can handle an aggregated maximum of 1M packets per second from all the monitored domain controllers. In some environments, the same ATP Center can handle overall traffic that is higher than 1M. Contact askcesec@microsoft.com for assistance with such environments.
...
> - It's possible to deploy the ATP Center on any IaaS vendor as long as the performance requirements that are described in this article are met.
...
> -   When working on a physical server, the ATP database needs you to **disable** Non-uniform memory access (NUMA) in the BIOS. Your system may refer to NUMA as Node Interleaving, in which case you have to **enable** Node Interleaving to disable NUMA. For more information, see your BIOS documentation. This is not relevant when the ATP Center is running on a virtual server.


## Choosing the right gateway type for your deployment
In an ATP deployment any combination of the ATP Gateway types is supported:

- Only ATP Gateways
- Only ATP Lightweight Gateways
...
|----|----|----|----|-----|
|ATP Gateway|The Out of band deployment makes it harder for attackers to discover ATP is present|Higher|Installed alongside the domain controller (out of band)|Supports up to 50,000 packets per second|
|ATP Lightweight Gateway|Doesn't require a dedicated server and port-mirroring configuration|Lower|Installed on the domain controller|Supports up to 10,000 packets per second|

The following are examples of scenarios in which domain controllers should be covered by the ATP Lightweight Gateway:
...
The following are examples of scenarios in which domain controllers should be covered by the ATP Gateway:


- Headquarter data centers (having domain controllers with more than 10,000 packets per seconds)


### ATP Lightweight Gateway Sizing

An ATP Lightweight Gateway can support the monitoring of one domain controller based on the amount of network traffic the domain controller generates. 
...
&#42;Total number of packets-per-second on the domain controller being monitored by the specific ATP Lightweight Gateway.

&#42;&#42;Total number of non-hyper threaded cores that this domain controller has installed.<br>While hyper threading is acceptable for the ATP Lightweight Gateway, when planning for capacity, you should count actual cores and not hyper threaded cores.

&#42;&#42;&#42;Total amount of memory that this domain controller has installed.

> [!NOTE]	
> -   If the domain controller does not have the resources required by the ATP Lightweight Gateway, domain controller performance is not effected, but the ATP Lightweight Gateway might not operate as expected.
> -   When running as a virtual machine dynamic memory or any other memory ballooning feature is not supported.
...
Consider the following issues when deciding how many ATP Gateways to deploy.

-	**Active Directory forests and domains**<br>
	ATP can monitor traffic from multiple domains from a single Active Directory forest. Monitoring multiple Active Directory forests requires separate ATP deployments. Do not configure a single ATP deployment to monitor network traffic of domain controllers from different forests.

-	**Port Mirroring**<br>
Port mirroring considerations might require you to deploy multiple ATP Gateways per data center or branch site.

-	**Capacity**<br>
	An ATP Gateway can support monitoring multiple domain controllers, depending on the amount of network traffic of the domain controllers being monitored. 
<br>
...
|20,000|6|24|
|50,000|16|48|
&#42;Total average number of packets-per-second from all domain controllers being monitored by the specific ATP Gateway during their busiest hour of the day.

&#42;The total amount of domain controller port-mirrored traffic cannot exceed the capacity of the capture NIC on the ATP Gateway.

&#42;&#42;Hyper-threading must be disabled.

> [!NOTE] 
> -   Dynamic memory is not supported.
> -   For optimal performance, set the **Power Option** of the ATP Gateway to **High Performance**.
> -   A minimum of 5 GB of space is required and 10 GB is recommended, including space needed for the ATP binaries, [ATP logs](troubleshooting-ata-using-logs.md), and [performance logs](troubleshooting-ata-using-perf-counters.md).
