...
- For sensor installations on Active Directory Federation Services (AD FS) servers, configure the SQL server to allow the *Directory service* account (**Configuration** > **Directory services** > **Username**) *connect*, *log in*, *read*, and *select* permissions to the **AdfsConfiguration** database. For additional instructions on how to grant access to the AD FS database, see [this explanation](/defender-for-identity/technical-faq#how-do-i-grant-access-to-the-ad-fs-database-if-i-m-using-the-windows-internal-database--wid-).
